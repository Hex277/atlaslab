...
function scrollToQrup(qrupId, clickedBtn) {
  ...
  function hesablaQrup1() {
    ...
    // DÜZƏLİŞ 1: ".umumi-netice-row" class-ı olan sətri seçmirik (Error verməməsi üçün)
    const elements = isMobile
      ? document.querySelectorAll("#qrup1 .mobile-card")
      : document.querySelectorAll("#qrup1 .qiymetlendirme tbody tr:not(.umumi-netice-row)");
    ...
    elements.forEach((el, index) => {
      ...
      // DÜZƏLİŞ 2: Əgər sətirdə inputlar yoxdursa, bu sətri atla (Təhlükəsizlik üçün)
      if (!dogruInput || !yanlisInput || !acikKodInput || !acikYaziliInput) return;
    ...
    // Ümumi nəticəni göstər (Artıq kod çökmədiyi üçün bura rəvan şəkildə çatacaq)
